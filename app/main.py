...
def display_analysis_results(result, is_image_analysis=False):
    ...
    if is_image_analysis:
        # Display image analysis results in cards
        st.markdown("<h2 style='color:#4a6fa5; margin-bottom:1.5rem;'>📸 Image Analysis Results</h2>", unsafe_allow_html=True)
        
        # Show scene description in a card
        if 'scene_description' in result and result['scene_description']:
            ...
            <div class="section-card">
                ...
                <p style="margin-bottom: 0;">{scene_desc}</p>
            </div>
            """, unsafe_allow_html=True)
        
        # Show detected objects in a card
        if 'detected_objects' in result and result['detected_objects']:
            objects_html = ""
            for obj in result['detected_objects']:
                confidence = float(obj.get('score', 0)) * 100
                objects_html += f"""
                <div style="margin: 0.75rem 0;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                        <span style="font-weight: 500;">{obj['label'].title()}</span>
                        <span style="font-weight: 600; color: #2c3e50;">{confidence:.1f}%</span>
                    </div>
                    <div style="height: 6px; background: #e9ecef; border-radius: 3px; overflow: hidden;">
                        <div style="height: 100%; width: {confidence}%; background: #4a6fa5;"></div>
                    </div>
                </div>
                """
            ...
            st.markdown(f"""
            <div class="section-card">
                <h3 class="section-title">📋 Detected Legal Concepts</h3>
                <div style="margin-top: 0.5rem;">{objects_html}</div>
            </div>
            """, unsafe_allow_html=True)
        
        # Show detected text in a card if any
        if 'detected_text' in result and result['detected_text']:
            text_items = "".join([f"<div class='example-incident' style='margin: 0.5rem 0;'>{text}</div>" for text in result['detected_text']])
            st.markdown(f"""
            <div class="section-card">
                <h3 class="section-title">✏️ Detected Text</h3>
    ...
    # For non-image analysis (text/voice), use the original styling
            ...
            for idx, section in enumerate(result['relevant_sections'], 1):
                ...
                <div class="section-box">
                    <div class="section-header">
                        <span>BNS Section {section.get('section_number', 'N/A')}: {section.get('section_title', 'Untitled')}</span>
                        <span style="margin-left: auto; font-weight: 500; color: #94a3b8;">
                            Relevance: {section.get('score', 0)*100:.1f}%
                        </span>
                    </div>
                ...
                if 'punishment' in section and section['punishment'] and section['punishment'] != 'Not specified':
                    ...
                    st.markdown("""
                    <div style="font-size: 0.85rem; color: #94a3b8; margin: 0.5rem 0 1.5rem 0; padding: 0.5rem; background-color: rgba(30, 41, 59, 0.5); border-radius: 4px; border-left: 2px solid #475569;">
    ...
    # For image analysis, use the new card-based styling
    else:
        # Display relevant BNS sections for image analysis
